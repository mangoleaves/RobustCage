...
void BSPcomplex::triangulateAllBspFaces()
{
  size_t numFaces = faces.size();
  for (uint64_t faceIdx = 0; faceIdx < numFaces; faceIdx++)
    triangulateFace(faceIdx);
}
...
void BSPcomplex::makeTetrahedra()
{
    ...
    for (uint64_t cell_i = 0; cell_i < cells.size(); cell_i++)
    {
      BSPcell& cell = cells[cell_i];
      if (cell.place != INTERNAL_A) continue;
      if (decomposition_type[cell_i] == 0)
      { // Simple tet
      ...
      else if (decomposition_type[cell_i] == 1)
      { // Tetrahedralizable from vertex
...
void BSPcomplex::saveMesh(const char* filename, const char bool_opcode, bool tetrahedrize)
{
    ...
    if (tetrahedrize)
    {
        ...
        for (size_t i = 0; i < vrts_visit.size(); i++) if (vrts_visit[i]) final_number++;
