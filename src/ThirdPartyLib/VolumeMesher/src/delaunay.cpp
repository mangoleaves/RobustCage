...
/// @brief
/// 1. calculate static filter.
/// 2. initialize first tetrahedron.
void TetMesh::init(){
  ...
  /// a Tetetrahedral is said to have positive volume
  /// is when it's face normal(calculated by right hand rule) 
  /// point to the opposite direction to the barycenter.
  ...
  ///    \  /
  ///     p4
  /// we say p4(p1) is opposite to p1(p4), p4(p1) is adjacent to p1(p4).
...
/// @brief given vertices, generate a tetrahedral mesh by
/// using "computing dirichlet tessellations(1981 bowyer)" method.
/// @attention before calling this method, initialize vertices and num_vertices.
...
/// @brief search a tetrahedron which contains vertex "v_id".
/// the search method looks like depth-first search.
/// @param [in] tet start tetrahedron
/// @param [in] v_id given vertex's id
/// @return a tetrahedron which contains vertex "v_id"
uint64_t TetMesh::searchTetrahedron(uint64_t tet, const uint32_t v_id)
{
    ...
    for (uint64_t f0 = 4;;) {
        ...
        uint64_t i = 0;
        // select vertex i and get the opposite triangle.
        // three vertices of the opp_tri must be in right order.
        // i | a | b | c
        // 0 | 1 | 3 | 2
        // 1 | 2 | 3 | 0
        // 2 | 3 | 1 | 0
...
/// @brief estimate whether a vertex "v_id" is contained by
/// a tet's circumscribed sphere.
/// @details a determinant is calculated to indicate containment.
/// Given tet with 4 verices a, b, c and d, we estimate whether
/// point e is contained by tet by calculating below determinant:
/// @verbatim 
/// | a.x   a.y   a.z   ||a||^2   1 |
/// | b.x   b.y   b.z   ||b||^2   1 |
/// | c.x   c.y   c.z   ||c||^2   1 |
...
/// | ae.x   ae.y   ae.z   ||ae||^2  |
/// @verbatim
/// where ab = b - a, in the same way we have ac, ad and ae.
/// @par Image that we have a circle "C" in 2D space, we add an extra dimension
/// and a quadratic surface "S" \f$ z=x^2+y^2 \f$ in higher dimensional space(3D space).
/// Then we project "C" to "S" and get an ellipse "E" on "S". The "E" also lies on a plane "PL".
/// @par Given a 2D point "p", we want to know whether "p" is inside "C".
/// We can project "p" to "S" to get "ps", if "ps" is under "PL", "ps" is inside "C", otherwise not.
/// @par We convert a InCircle problem to Orient problem. Finally, we have determinant above.
/// (by extending 2D case to 3D case, we can solve InSphere problem.)
...
/// @brief push node and vertex to "Del_tmp".
/// but...what is bnd?
...
/// @brief Find all tets whose circumscribed sphere contains the vertex.
/// And put these tets into "Del" arrays. The search method is WFS.
void TetMesh::deleteInSphereTets(uint64_t tet, const uint32_t v_id)
{
  uint64_t start;
  Del_deleted[Del_num_deleted++] = tet; // push into "Del" array.
  tet_subdet[tet + 3] = -1.0;           // marked as deleted.
  ...
  for(start=Del_num_deleted-1; start < Del_num_deleted; start++){
    ...
    // allocate more memory if neccessary.
    ...
    // estimate whether vertex "v_id" contained by neighbor tet's circumscribed sphere.
    // if contained, delete the neighbor tet;
    ...
    // below are other three neighbor tets.
...
/// @brief tetrahedralize hole that generated by deleting old tets.
/// new tets that fill the hole are generated by connecting
/// boundary face of hole and inserted vertex.
/// The filling procedure are memory efficient.(so it's confusing.)
void TetMesh::tetrahedrizeHole(uint64_t* tet)
{
  uint64_t clength = Del_num_deleted;
  uint64_t blength = Del_num_tmp;
  // Move tets in Del_tmp to deleted tets' position.
  // If deleted tets' positions are not enought,
  // allocate more positions to store tets in Del_tmp.
  if (blength > clength)
  {
  ...
  // Move tets in Del_tmp to deleted tets' position.
  // The indices in Del_deleted point to position of
  // deleted tets in tet_nodes array.
  // Del_tmp[0...blength] --are moved to-> Del_deleted[start...clength]
  for (uint64_t i = 0; i < blength; i++)
  {
    ...
    // of the current deleted tetrahedron to the tetrahedron itself
    if(tet_node[tet+3]!=INFINITE_VERTEX)            // Mod.1
      for(uint32_t j=0; j<4; j++)
          vertices[tet_node[tet + j]].inc_tet = tet>>2;
  }
  // After moving new tets to old positions or new positions (above codes),
  // generate new incident relations between new tets.
  uint64_t tlength = 0;
