...
public:
  ...
  uint64_t* tet_neigh;       // Adjacent tetrahedron array, see init().
  ...
  {
      return n[i] & 0xFFFFFFFFFFFFFFFC; // same as "% 4".
  }
...
protected:
    ...
    /// "Del_tmp" temporarily stores new tets.
    /// These new tets are generated by connecting
    /// existed triangle face(boundary of hole)
    /// and current inserted vertex.

    uint64_t Del_num_tmp;
    uint64_t Del_size_tmp;
    /// "Del_deleted" stores index of deleted tets.
    /// The index points to a position in "tet_node" array.
