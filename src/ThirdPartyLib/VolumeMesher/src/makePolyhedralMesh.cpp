...
/// @brief remove duplacated points. two points are considered same when
/// their coordinates are exactly equal.
/// @param [out] vertices_p \b sorted unique vertices.
/// @param [out] npts number of vertices_p.
/// @param [in] vrts_copy original vertices that may duplicate.
/// @param [out] map map original vertices' indices to sorted unique vertices' indices.
/// @param [out] diff number of duplacated points before sorted unique vertices' index.
...
/// @brief remove duplate vertices, and modify triangles' points' indices.
/// @param [in] coords_A vertices' coordinates.
/// @param [in] npts_A number of input vertices. (note: not number of coordinates, which is 3 times of nPnts).
/// @param [in] tri_idx_A triangles' points' indices.
/// @param [in] ntri_A number of input triangles.
/// @param [out] vertices_p sorted unique vertices.
/// @param [out] npts number of vertices_p.
/// @param [out] tri_vertices_p modified triangles' points' indices.
/// @param [out] ntri number of tri_vertices_p .
/// @param [in] verbose whether logging.
void read_nodes_and_constraints(double* coords_A, uint32_t npts_A, uint32_t* tri_idx_A, uint32_t ntri_A,
    ...
    *tri_vertices_p = (uint32_t*)malloc(sizeof(uint32_t) * 3 * (*ntri));
    ...
    // 1. find bounding box for surface mesh and add points on box corner
    // 2. add points around surface mesh
